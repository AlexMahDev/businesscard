...
class ContactRepository {
  Future<List<ContactModel>> getContacts() async {
    ...
    try {
      final contactsReq = await FirebaseFirestore.instance
          .collection("users")
          .doc(user.uid)
          .collection("contacts")
          .orderBy('timestamp')
          .get();
      return contactsReq.docs.isNotEmpty
          ? List.from(
              contactsReq.docs.map((e) => ContactModel.fromJson(e.data())))
          : [];
    } catch (e) {
      throw Exception(e);
    }
  ...
  Future<void> saveContact(CardModel card) async {
    ...
    try {
      final contactFirebase = FirebaseFirestore.instance
          .collection("users")
          .doc(user.uid)
          .collection("contacts")
          .doc(card.cardId);
